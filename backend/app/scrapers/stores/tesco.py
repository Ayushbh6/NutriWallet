"""Tesco UK store parser with CSS selector-first approach."""
...
class TescoParser:
    """Parser for Tesco.com product pages."""

    # CSS selectors for Tesco product pages
    SELECTORS = {
        "product_name": [
    ...
    def parse(self, html_content: str, url: Optional[str] = None) -> Optional[Dict]:
        ...
        try:
            ...
            if not product_name or not price_str:
                logger.warning("tesco_parser_failed", url=url)
                return None
            ...
            if not price:
                logger.warning("tesco_parser_price_normalization_failed", price_str=price_str)
                return None
            ...
            result = {
                ...
                "currency": "GBP",  # Tesco is UK, default to GBP
            }

            logger.debug("tesco_parser_success", product=product_name, price=price)
            return result

        except Exception as e:
            logger.error("tesco_parser_error", error=str(e), url=url)
            return None
    ...
    def _extract_product_name_fallback(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        if title:
            ...
            title_text = re.sub(r'\s*-\s*Tesco.*$', '', title_text, flags=re.IGNORECASE)
            return title_text
    ...
    def _extract_price_fallback(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        # Look for price patterns in text (GBP format)
        price_patterns = [
            r'£\s*(\d+[.,]\d{2})',
            r'(\d+[.,]\d{2})\s*£',
            r'(\d+[.,]\d{2})',
    ...
    def _extract_unit_fallback(self, soup: BeautifulSoup, price_str: Optional[str]) -> Optional[str]:
        ...
        unit_patterns = [
            r'(\d+)\s*(kg|g|L|l|ml|mL|piece|each)',
            r'per\s*(kg|g|L|l|ml|mL|piece|each)',
            r'/\s*(kg|g|L|l|ml|mL|piece|each)',
        ]
