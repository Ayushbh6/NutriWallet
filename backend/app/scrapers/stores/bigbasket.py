"""BigBasket India store parser with CSS selector-first approach."""
...
class BigBasketParser:
    """Parser for BigBasket.com product pages."""

    # CSS selectors for BigBasket product pages
    SELECTORS = {
        "product_name": [
    ...
    def parse(self, html_content: str, url: Optional[str] = None) -> Optional[Dict]:
        ...
        try:
            ...
            if not product_name or not price_str:
                logger.warning("bigbasket_parser_failed", url=url)
                return None
            ...
            if not price:
                logger.warning("bigbasket_parser_price_normalization_failed", price_str=price_str)
                return None
            ...
            result = {
                ...
                "currency": "INR",  # BigBasket is India, default to INR
            }

            logger.debug("bigbasket_parser_success", product=product_name, price=price)
            return result

        except Exception as e:
            logger.error("bigbasket_parser_error", error=str(e), url=url)
            return None
    ...
    def _extract_product_name_fallback(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        if title:
            ...
            title_text = re.sub(r'\s*-\s*BigBasket.*$', '', title_text, flags=re.IGNORECASE)
            return title_text
    ...
    def _extract_price_fallback(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        # Look for price patterns in text (INR format)
        price_patterns = [
            r'₹\s*(\d+(?:[.,]\d{1,2})?)',
            r'Rs\.?\s*(\d+(?:[.,]\d{1,2})?)',
            r'(\d+(?:[.,]\d{1,2})?)',
            ...
            if matches:
                ...
                for match in matches:
                    try:
                        ...
                        if 1 <= price_val <= 100000:  # Reasonable price range for INR
                            return match
                    except ValueError:
                        continue
    ...
    def _extract_unit_fallback(self, soup: BeautifulSoup, price_str: Optional[str]) -> Optional[str]:
        ...
        unit_patterns = [
            r'(\d+)\s*(kg|g|L|l|ml|mL|piece|each|pack)',
            r'per\s*(kg|g|L|l|ml|mL|piece|each|pack)',
            r'/\s*(kg|g|L|l|ml|mL|piece|each|pack)',
        ]
